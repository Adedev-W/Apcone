...
def process_uploaded_document(job_id: str) -> dict[str, object]:
    ...
    try:
        ...
        job = service.mark_job_running(job_uuid, parser_name="pending")
        ...
        service.update_job_progress(job_uuid, progress=100, parser_name=extracted.parser_name, page_count=extracted.page_count)
        return {
            "job_id": str(result.job.id),
            "document_id": str(result.document.id),
            "status": result.job.status.value,
